...
def test_cached_turns_survive_a_round_trip(tmp_path):
    ...
    diarization.save_cached(path, TURNS)
    assert diarization.load_cached(path) == TURNS
...
def test_a_second_run_reuses_the_turns_instead_of_the_gpu(tmp_path):
    ...
    with isolated_diarization_cache():
        diarization.save_cached(diarization.cache_path(str(audio), 0, 0), TURNS)
        ...
        logs: list[str] = []
        turns = diarizer.diarize(str(audio), 0, 0, log_cb=logs.append)
...
def test_prune_keeps_only_the_most_recent(tmp_path):
    with isolated_diarization_cache() as cache:
        ...
        for i in range(5):
            diarization.save_cached(cache / f"{i}.json", TURNS)
